...
class Twint:
    ...
    async def Lookup(self):
        ...
        try:
            ...
            logme.exception(__name__ + ":Twint:Lookup:Unexpected opse_exception occurred.")
            raise
...
def run(config, callback=None):
    ...
    try:
        ...
        if "no current event loop" in str(e):
            ...
            logme.exception(
                __name__
                + ":run:Unexpected opse_exception while handling an expected RuntimeError."
            )
            raise
    except Exception as e:
        logme.exception(
            __name__
            + ":run:Unexpected opse_exception occurred while attempting to get or create a new event loop."
        )
        raise
