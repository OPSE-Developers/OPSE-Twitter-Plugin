...
def Tweet(tw, config):
    ...
    # Added by OPSE developpers
    t.auth_user_id = t.user_id
    t.auth_username = t.username
    t.auth_name = t.name
    # ***
    ...
    try:
        if "result" in tw["retweeted_status_result"]:
            t.retweet = True
            t.retweet_id = int(tw["retweeted_status_result"]["result"]["legacy"]["id_str"])
            t.user_rt_name = tw["retweeted_status_result"]["result"]["core"]["user_results"]["result"]["legacy"]["name"]
            t.user_rt_username = tw["retweeted_status_result"]["result"]["core"]["user_results"]["result"]["legacy"]["screen_name"]
            t.user_rt_id = int(tw["retweeted_status_result"]["result"]["legacy"]["user_id_str"])
            t.auth_user_id = t.user_rt_id
            t.auth_username = t.user_rt_username
            t.auth_name = t.user_rt_name
    except KeyError:
        t.retweet = False
        t.retweet_id = ""
        t.retweet_date = ""
